...
class AV2SensorDataLoader:
    ...
    def get_labels_at_lidar_timestamp_all(self, annotations_feather_path, log_id, lidar_timestamp_ns: int, get_moving: bool) -> CuboidList:
        cuboid_list = CuboidList.from_feather_all(annotations_feather_path, log_id, get_moving)
        cuboids = list(filter(lambda x: x.timestamp_ns == lidar_timestamp_ns, cuboid_list.cuboids))
        return CuboidList(cuboids=cuboids)
