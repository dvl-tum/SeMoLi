...
WAYMO_CLASSES = {'TYPE_UNKNOWN': 0, 'TYPE_VECHICLE': 1, 'TYPE_PEDESTRIAN': 2, 'TYPE_SIGN': 12, 'TYPE_CYCLIST': 15}
...
def get_feather_files(
    ...
    if not is_gt:
        if 'score' not in df.columns:
            df['score'] = [1] * df.shape[0] # np.max(df['num_interior_pts'].values)/df['num_interior_pts'].values # [1] * df.shape[0]
    ...
    if is_gt:
        ...
        file = str(remove_non_move_thresh)[:5] + '_' + file if remove_non_move else file
            ...
            for i, t in enumerate(sorted(seq_df['timestamp_ns'].unique().tolist())):
                ...
                if remove_non_move:
                    if remove_non_move_strategy == 'per_seq':
                        ...
                        ego_t1_SE3s_obj_t1 = [label.dst_SE3_object for label in labels]

                        if i < len(timestamps) - 1:
                            # labels at t+1 
                            labels_t2 = loader.get_labels_at_lidar_timestamp(
                                log_id=seq, lidar_timestamp_ns=int(timestamps[i+1]))
                            ego_t2_SE3s_obj_t2 = [label_t2.dst_SE3_object for label_t2 in labels_t2]
                            city_SE3_t2 = loader.get_city_SE3_ego(seq, int(timestamps[i+1]))
                        ...
                        if i > 0:
                            labels_t0 = loader.get_labels_at_lidar_timestamp(
                                log_id=seq, lidar_timestamp_ns=int(timestamps[i-1]))
                            ego_t0_SE3s_obj_t0 = [label_t0.dst_SE3_object for label_t0 in labels_t0]
                            city_SE3_t0 = loader.get_city_SE3_ego(seq, int(timestamps[i-1]))
                        ...
                        city_SE3_t1 = loader.get_city_SE3_ego(seq, int(timestamps[i]))
                        ...
                        for m, label in enumerate(labels_city):
                            if len(labels_t2) and label.track_id in ids_t2:
                                # ego_t2_SE3_ego_t1 = city_SE3_t2.inverse().compose(city_SE3_t2)
                                # obj_t1_ego_t2 = ego_t2_SE3_ego_t1.transform_point_cloud(label.dst_SE3_object.translation)
                                # obj_t2_ego_t2 = ego_t2_SE3s_obj_t2[ids_t2.index(label.track_id)].compose(
                                #     label.dst_SE3_object.inverse()).transform_point_cloud(label.dst_SE3_object.translation)
                                
                                # translation = obj_t2_ego_t2 - obj_t1_ego_t2
                                # dist = np.linalg.norm(translation)

                                other_trans = labels_t2[ids_t2.index(label.track_id)].dst_SE3_object.translation
                                dist = np.linalg.norm(label.dst_SE3_object.translation[:-1] - other_trans[:-1])
                                
                                if 'argo' in path:
                                    diff_time = (timestamps[i+1]-t) / np.power(10, 9)
                                else:
                                    diff_time = (timestamps[i+1]-t) / np.power(10, 6)

                                vel = dist/diff_time

                                if t == 1543280295123930:
                                    print(labels[m].dst_SE3_object.translation[:-1], label.dst_SE3_object.translation[:-1], other_trans[:-1], vel)
                                
                                vels.append(vel)
                                bool_labels.append(vel > remove_non_move_thresh)
                            
                            elif len(labels_t0) and label.track_id in ids_t0:
                                # ego_t0_SE3_ego_t1 = city_SE3_t0.inverse().compose(city_SE3_t0)
                                # obj_t1_ego_t0 = ego_t0_SE3_ego_t1.transform_point_cloud(label.dst_SE3_object.translation)
                                # obj_t0_ego_t0 = ego_t0_SE3s_obj_t0[ids_t0.index(label.track_id)].compose(
                                #     label.dst_SE3_object.inverse()).transform_point_cloud(label.dst_SE3_object.translation)

                                # translation = obj_t0_ego_t0 - obj_t1_ego_t0
                                # dist = np.linalg.norm(translation)
                                ...
                                if 'argo' in path:
                                    ...
                                    diff_time = (t-timestamps[i-1]) / np.power(10, 6)
                                ...
                                if t == 1543280295123930:
                                    print(labels[m].dst_SE3_object.translation[:-1], label.dst_SE3_object.translation[:-1], None)
                                vels.append(None)
                                bool_labels.append(False)

                        labels = [l for i, l in enumerate(labels) if bool_labels[i]]
                    else:
                        assert remove_non_move_strategy in ['per_frame', 'per_seq'], 'remove strategy for static objects not defined'
        ...
        for i, timestamp in enumerate(sorted(ddf['timestamp_ns'].unique())):
            ...
            for i, row in time_gdf.iterrows():
                if row['category'] == 'PEDESTRIAN':
                    ...
                    marker = 'o'
                plt.scatter(row['tx_m'], row['ty_m'], color='black', marker=marker)
                x_0 = row['tx_m']-0.5*row['length_m']
                y_0 = row['ty_m']-0.5*row['width_m']
                rect = patches.Rectangle(
                    (x_0, y_0),
            ...
            plt.xlim(x_lim)
            plt.ylim(y_lim)
            plt.savefig(f'../../../Visualization_Whole_DETS/{seq}/frame_{timestamp}_{name}.jpg')
            plt.close()
...
def eval_detection(
    ...
    loader = AV2SensorDataLoader(data_dir=Path(gt_folder), labels_dir=Path(gt_folder))
    dataset_dir = Path(gt_folder)
    eval_only_roi_instances = False if 'waymo' in gt_folder else True
    competition_cfg = DetectionCfg(dataset_dir=dataset_dir, eval_only_roi_instances=eval_only_roi_instances)  # Defaults to competition parameters.
    if just_eval:
        print("Loading data...")
