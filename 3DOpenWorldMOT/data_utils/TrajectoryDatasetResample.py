...
class TrajectoryDataset(PyGDataset):
    def __init__(
        ...
        self.resample_radius = resmaple_radius
    ...
    def resample(self, path, data):
        ...
        feather_path = os.path.join(
            ...
            f'{data.log_id}',
            'sensors/lidar',
            f'{data.timestamps[0]}.feather')
        f = feather.read_feather(feather_path)

        # get data withing resampling area
        pos_non_cd_filtered = f[
            np.logical_and(
                np.logical_and(f['cd_dist_4']>self.cd_filter, f['cd_dist_4']<=0.2), 
                f['non_ground_pts_rc'])]
        pos_non_cd_filtered = pos_non_cd_filtered[['x', 'y', 'z']].values
        pos_non_cd_filtered = torch.from_numpy(pos_non_cd_filtered)
        
        # kNN within radius 1m around points
        num_neighbors = 64
        dist = torch.cdist(data.pc_list.unsqueeze(0), pos_non_cd_filtered.unsqueeze(0)).squeeze()
        idx_0 = torch.tile(torch.arange(dist.shape[0]).unsqueeze(1), (1, num_neighbors)).flatten()
        idx_1 = dist.topk(k=num_neighbors, dim=1, largest=False).indices.flatten()

        idx_mask = dist[idx_0, idx_1] < self.resample_radius
        idx_0 = idx_0[idx_mask]
        idx_1 = idx_1[idx_mask]
        d_sorted = torch.unique(idx_1)
        
        # resample
        data['resampled'] = torch.cat([torch.zeros(data.pc_list.shape[0]), torch.ones(d_sorted.shape[0])])
        data['pc_list'] = torch.vstack([data.pc_list, pos_non_cd_filtered[d_sorted]])
        import torch_scatter
        idx_1_traj = idx_1.unsqueeze(1).unsqueeze(1).expand((idx_1.shape[0], data['traj'].shape[1], data['traj'].shape[2]))
        traj_sampled = torch_scatter.scatter_mean(data['traj'][idx_0], idx_1_traj[:], dim=0)[d_sorted]
        point_instances_sampled = torch_scatter.scatter_mean(data['point_instances'].squeeze()[idx_0], idx_1).long()[d_sorted].unsqueeze(0)
        point_categories_sampled = torch_scatter.scatter_mean(data['point_categories'].squeeze()[idx_0], idx_1).long()[d_sorted].unsqueeze(0)
        point_instances_mov_sampled = torch_scatter.scatter_mean(data['point_instances_mov'].squeeze()[idx_0], idx_1).long()[d_sorted].unsqueeze(0)
        point_categories_mov_sampled = torch_scatter.scatter_mean(data['point_categories_mov'].squeeze()[idx_0], idx_1).long()[d_sorted].unsqueeze(0)
        
        # assign new resampled data
        data['traj'] = torch.vstack([data['traj'], traj_sampled])
        data['point_instances'] = torch.hstack([data['point_instances'], point_instances_sampled])
        data['point_categories'] = torch.hstack([data['point_categories'], point_categories_sampled])
        data['point_instances_mov'] = torch.hstack([data['point_instances_mov'], point_instances_mov_sampled])
        data['point_categories_mov'] = torch.hstack([data['point_categories_mov'], point_categories_mov_sampled])
    ...
    def get(self, idx):
        ...
        if not self.use_all_points and data['traj'].shape[0] > self.num_points:
            ...
            if 'resampled' in data.keys:
                data['resampled'] = data['resampled'][idxs]
...
def get_TrajectoryDataLoaderResample(cfg, name=None, train=True, val=True, test=False):
    ...
    if val:
        if 'evaluation' in cfg.data.detection_set:
            ...
            split = 'train'
        val_data = TrajectoryDataset(cfg.data.data_dir + f'_{split}/'  if 'Argo' not in cfg.data.data_dir else cfg.data.data_dir,
                split,
                ...
                resmaple_radius=cfg.data.resmaple_radius)
    else:
        val_data = None
    if test:
        split = 'test'
