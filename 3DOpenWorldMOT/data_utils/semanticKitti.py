...
class SemanticKitti(PyGDataset):
    LABELS = LABELS
    COLOR_MAP = COLOR_MAP
    CONTENT = CONTENT
    ...
    def __init__(self, root, split="trainval", transform=None, process_workers=1, pre_transform=None):
        ...
        super().__init__(self)
        if split == "train":
            self._scans = glob(osp.join(self.processed_paths[0], "*.pt"))
        elif split == "val":
            self._scans = glob(osp.join(self.processed_paths[1], "*.pt"))
        elif split == "test":
            self._scans = glob(osp.join(self.processed_paths[2], "*.pt"))
        elif split == "trainval":
            self._scans = glob(osp.join(self.processed_paths[0], "*.pt")) + glob(
                osp.join(self.processed_paths[1], "*.pt")
            )
        else:
    ...
    def _load_paths(self, seqs):
        ...
        for seq in seqs:
            scan_paths.extend(
                sorted(glob(osp.join(self.raw_paths[0], "{0:02d}".format(int(seq)), "velodyne", "*.bin")))
            )
            label_path.extend(
                sorted(glob(osp.join(self.raw_paths[0], "{0:02d}".format(int(seq)), "labels", "*.label")))
            )
    ...
    def process_one(scan_file, label_file, transform, out_file):
        ...
        if transform:
            data = transform(data)
        log.info("Processed file %s, nb points = %i", osp.basename(out_file), data.pos.shape[0])
    ...
    def process(self):
        for i, split in enumerate(self.AVAILABLE_SPLITS[:-1]):
            ...
            for scan in scan_paths:
                scan = osp.splitext(scan)[0]
                seq, _, scan_id = scan.split(osp.sep)[-3:]
                scan_names.append("{}_{}".format(seq, scan_id))

            out_files = [osp.join(self.processed_paths[i], "{}.pt".format(scan_name)) for scan_name in scan_names]
...
if __name__ == "__main__":
    ...
    dataroot = osp.join(DIR, "..", "..", "data", "kitti")
    SemanticKitti(
